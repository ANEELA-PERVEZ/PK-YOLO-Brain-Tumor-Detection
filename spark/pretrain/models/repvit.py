...
class RepViT(nn.Module):
    def __init__(self, cfgs, distillation=False, pretrained=None, init_cfg=None, out_indices=[]):
        ...
        self.train()
        self.out_indices = [0, 1, 7, 14, 50]
        #               320 160 80  40    20
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(RepViT, self).train(mode)

    # Line 225: Sirf RepViT class ka forward function badlein
    def forward(self, x, hierarchical=False):
        ...
        # SparK pretraining ke liye humein list ko reverse karke bhejna hoga
        # taake 640 channels wala feature pehle aaye (Smallest Res)
        if hierarchical:
            return outs
        
        return outs

    # Ye function class ke andar hone chahiye (aik tab ka gap dein)
    def get_downsample_ratio(self) -> int:
        return 32 

    def get_feature_map_channels(self) -> list:
        return [40, 80, 160, 320, 640]
# RepViT class ke andar (forward function ke baad) ye add karein:
    
    def get_downsample_ratio(self) -> int:
        """RepViT ka final downsampling ratio 32 hai."""
        return 32 

    def get_feature_map_channels(self) -> list:
        """Aapke model ke 4 main stages ke channels jo aapne code mein define kiye hain."""
        return [40, 80, 160, 320, 640]
...
from timm.models.registry import register_model # Agar upar nahi likha toh ye add karein

@register_model # Ye line zaroor add karein
