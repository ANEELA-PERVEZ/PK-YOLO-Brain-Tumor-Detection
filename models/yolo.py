...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (DualDetect, TripleDetect, DualDDetect, TripleDDetect)):
            ...
            with torch.no_grad():
                dummy_out = forward(torch.zeros(1, ch, s, s))
                # Agar output nested list hai (DualDDetect case), toh pehli branch use karein
                if isinstance(dummy_out, list) and isinstance(dummy_out[0], list):
                    m.stride = torch.tensor([s / x.shape[-2] for x in dummy_out[0]])
                else:
                    m.stride = torch.tensor([s / x.shape[-2] for x in dummy_out])
            self.stride = m.stride
            m.bias_init()
        # Init weights, biases
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
       ...
       # YOLO11 modules (C3k2, PSA) ko yahan add kiya gaya hai
        if m in {
            Conv, AConv, ConvTranspose, 
            Bottleneck, SPP, SPPF, DWConv, BottleneckCSP, nn.ConvTranspose2d, DWConvTranspose2d, SPPCSPC, ADown,
            RepNCSPELAN4, SPPELAN, C3k2, PSA, RepViTBlock, RepViTSEBlock}:
            c1, c2 = ch[f], args[0]
...
# --- YOLO11 & RepViT Custom Modules for PK-YOLO ---

class C3k2(nn.Module):
    # YOLO11 C3k2 module for faster and accurate feature fusion
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, self.c, 1, 1)
        self.cv2 = Conv(c1, self.c, 1, 1)
        # Fixed: Removed incorrect asterisk usage
        self.m = C3k(self.c, self.c, n, c3k, g, shortcut) if n > 0 else nn.Identity()
...
class C3k(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        self.m = nn.Sequential(*(Bottleneck(c1, c2, shortcut, g, k=(3, 3), e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.m(x)

class PSA(nn.Module):
    # Position Sensitivity Attention (PSA) module from YOLO11
    def __init__(self, c1, c2, e=0.5):
        ...
        self.attn = nn.Sequential(
            ...
            nn.Conv2d(2 * self.c, 2 * self.c, 1, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x):
        return x + self.cv2(self.attn(self.cv1(x)) * self.cv1(x))

# Paper specific blocks if not already in common.py
class RepViTBlock(nn.Module):
    def __init__(self, c1, c2=None):
        super().__init__()
        # Placeholder for RepViT logic - ensure this matches your mapped weights
        self.conv = Conv(c1, c1, 3, 1)
    def forward(self, x):
        return self.conv(x)

class RepViTSEBlock(nn.Module):
    def __init__(self, c1, c2=None):
        super().__init__()
